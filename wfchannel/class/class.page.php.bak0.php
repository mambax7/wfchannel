    ...
    public function getHtml()
    {
        if ($this->getVar('wfc_file') == 'http://' or !$this->getVar('wfc_file')) {
            ...
            if (is_readable($maintextfile)) {
                ...
                $_maintext = _MD_WFCHANNEL_FILEERROR;
            }
        }
...
class wfc_PageHandler extends wfp_ObjectHandler
{
    ...
    {
        parent::__construct($db, 'wfcpages', 'wfc_Page', 'wfc_cid', 'wfc_title', 'wfc_page');
    }
     ...
     * @return
     */
    public function &getHtml($file = '', $cleanlevel = '')
    {
     ...
     * @return
     */
    public function &cleanUpHTML($text, $cleanlevel)
    {
     ...
     * @return
     */
    public function getMainContent(&$_page_obj, $ret)
    {
     ...
     * @return
     */
    public function &setSearch($headline, $maintext)
    {
        ...
        for ($i = 0; $i < count($ascii_array); ++$i) {
            $search = preg_replace('#' . $html_array[$i] . '#', $chars_array[$i], $search);
        }
        $search = preg_replace("#<br />#", " ", $search);
        // elimine des faux positifs
        $search = preg_replace("#-//W3C//DTD HTML 4.0 Transitional//EN#", " ", $search);
        $search = preg_replace("/name=AUTHOR/", " ", $search);
        $search = preg_replace("/name=COPYRIGHT/", " ", $search);
        $search = preg_replace("/name=DESCRIPTION/", " ", $search);
        $search = preg_replace("/name=GENERATOR/", " ", $search);
        $search = preg_replace("/border=0/", " ", $search);
    ...
    public function &getSearch($queryarray, $andor, $limit, $offset)
    {
        $criteria = new CriteriaCompo();
        if ($andor != 'exact') {
            $andor = $andor;
     ...
     * @return
     */
    public function headingHtml($total_count)
    {
        /**
        ...
        $ret .= "</div>";
        $ret .= '<form><div style="text-align: left; margin-bottom: 12px;"><input type="button" name="button" onclick=\'location="index.php?op=edit"\' value="' . _MD_WFP_CREATENEW . '"></div></form>';
        $ret .= "<div style='text-align: right'>" . _MD_WFP_DISPLAYAMOUNT_BOX . wfp_getSelection($list_array, $nav['limit'], "limit", 1, 0, false, false, "onchange=\"location='index.php?limit='+this.options[this.selectedIndex].value\"", 0, false) . "</div>";
        echo $ret;
        unset($refer_handler);
    }
     ...
     * @return
     */
    public function pdfCleaner($text)
    {
        $text = str_replace('<p>', "\n\n", $text);
        $text = str_replace('<P>', "\n\n", $text);
        $text = str_replace('<br />', "\n", $text);
