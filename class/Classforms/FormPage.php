...
if (!$this->isNew()) {
    //mb
    echo '<div style="padding: 5px; float: left;"><a target="_BLANK" href="../index.php?cid=' . $this->getVar('wfc_cid') . '">' . \_AM_WFC_QUICKVIEW . '</a></div>';
}


class FormPage extends Xoopsforms\XoopsThemeTabForm
{
    public $targetObject;
    public $helper;
...
//        $form = new XoopsThemeTabForm((!$this->isNew()) ? \sprintf(\_AM_WFP_MODIFY, $this->getVar('wfc_title')) : \_AM_WFP_CREATE, 'page_form', 'main.php'); //mb
        ...
        $title = $this->targetObject->isNew() ? sprintf(AM_OBITUARIES2_OBITUARY_ADD) : sprintf(AM_OBITUARIES2_OBITUARY_EDIT);
        ...
        $this->addElement(new \xoopsFormHidden('wfc_cid', $this->getVar('wfc_cid')));

        /**
         * First Fields
         */
        $this->startTab(\_AM_WFC_TABMAIN, 'main-info');
        /**
         * Page Title
         */
        ...
        $this->addElement($page_title, true);
        /**
         * Page Headline
         */
        ...
        $this->addElement($page_subtitle, false);
        ...
        $wfc_content = new \XoopsFormEditor('', Wfresource\Utility::getModuleOption('use_wysiwyg'), $options, $nohtml = false, $onfailure = 'textarea');
        $options_tray->addElement($wfc_content);
        // $options_tray->setDescription( _AM_EWFC_TEXTCONTENT_DSC );

        if (Wfresource\Utility::isEditorHTML()) {
        ...
        $this->addElement($clean_select);

        /**
         * Buttons
         */
        $this->addElement(new \XoopsFormButtonTray('submit', _SUBMIT));
        $this->endTab();

        /**
         * Others
         */
        $this->startTab(\_AM_WFC_TABPUBLISH, 'main-published');
        ...
        $this->addElement($page_author, false);
        /**
         * weight
         */
        ...
        $this->addElement($page_weight, false);
        ...
        $this->addElement($wfc_allowcomments);
        ...
        $this->startTab(\_AM_WFC_TABIMAGE, 'main-image');
        /**
         * Page Image
         */
        ...
        $this->addElement($page_image, false);
        ...
        $this->addElement($page_imgwidth, false);
        ...
        $this->addElement($page_imgheight, false);
        ...
        $this->addElement($page_caption, false);

        $this->addElement(new \XoopsFormButtonTray('submit', _SUBMIT));
        $this->endTab();

        $this->startTab(\_AM_WFC_TABMETA, 'main-meta');
        /**
         * Meta tags title
         */
        ...
        $this->addElement($wfc_metakeywords);
        /**
         * Meta tags title
         */
        ...
        $this->addElement($wfc_related, false);

        if (\class_exists(\XoopsModules\Tag\FormTag::class) && xoops_isActiveModule('tag')) {
            $item_tag = new \XoopsModules\Tag\FormTag('item_tag', 60, 255, $this->getVar('wfc_cid'), 0);
            $item_tag->setDescription(\_AM_EWFC_METATITLE_DSC);
            $this->addElement($item_tag);
        }
         ...
         * Permissions
         */
        $this->startTab(\_AM_WFC_TABPERMISSIONS, 'main-permissions');
        ...
        $groups = new Xoopsforms\XoopsFormSelectCheckGroup(\_AM_EWFP_GROUPS, 'page_read', $group->getAdmin($this->getVar('wfc_cid')), '', true);
        ...
        if (!$this->isNew()) {
            ...
            $this->startTab(\_AM_WFC_INFO, 'main-information');
            ...
            $this->addElement($page_info, false);
